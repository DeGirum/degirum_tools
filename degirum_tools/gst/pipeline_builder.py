#
# pipeline_builder.py: GStreamer pipeline string builder for various video sources
#
...
This module provides ``build_gst_pipeline``, which automatically constructs a
``gst-launch``-style pipeline string for the most common video source types.

Key Features

- **Camera devices** — detects whether a V4L2 device is a Raspberry Pi CSI camera
  (using ``v4l2-ctl`` and ``/sys`` inspection) or a standard USB webcam, and picks the
  appropriate source element (``libcamerasrc`` or ``v4l2src``).
- **RTSP streams** — builds an ``rtspsrc`` pipeline that selects only the video RTP pad
  so that audio streams do not cause ``not-linked`` errors downstream.
- **Video files** — uses ``filesrc ! decodebin`` for broad codec compatibility.
- **Custom pipeline strings** — strings that already look like ``gst-launch`` pipelines
  (contain ``!`` and GStreamer element names) are passed through unchanged.
- **Platform detection** — reads ``/proc/device-tree/model`` and ``/proc/cpuinfo`` to
  identify Raspberry Pi, Jetson, Intel, and AMD hosts so source selection can be
  platform-aware.

Typical Usage

Pass any supported source to ``build_gst_pipeline`` and feed the returned string
directly into ``GstPipelineHandler``::
    ...
    # From a camera index
    pipeline_str = build_gst_pipeline(0)

    # From an RTSP URL
    pipeline_str = build_gst_pipeline("rtsp://192.168.1.10/stream")

    # From a file
    pipeline_str = build_gst_pipeline("/recordings/clip.mp4")
...
- ``build_gst_pipeline(source)`` — return a ``gst-launch``-style pipeline string for
  the given source (``int`` device index, RTSP URL string, file path string, or a
  verbatim pipeline string).
"""
