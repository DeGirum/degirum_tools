...
def detect_camera_type(device_index: int) -> str:
    ...
    if ret == 0:
        ...
        # logger.info(f"v4l2-ctl output for {device_path}: {stdout}")
    ...
    try:
        ...
        if os.path.exists(sys_path):
            ...
            # logger.info(f"Device sys path for {device_path}: {real_path}")
...
def build_gst_pipeline(source):
    ...
    if isinstance(source, int) or (isinstance(source, str) and source.isdigit()):
        ...
        camera_device = detect_camera_type(int(source))

        print(f"Detected platform: {platform}, camera type: {camera_device}")

        if camera_device == "rpi_csi" and platform == "raspberrypi":
            # Raspberry Pi CSI Camera
            if check_element_exists("libcamerasrc"):
                logger.info("Using libcamerasrc for RPi CSI camera")
                return "libcamerasrc ! videoconvert ! video/x-raw, format=RGB ! appsink name=sink"
            else:
                logger.warning("libcamerasrc not available, trying v4l2src")
                # Fallback to v4l2src

        # USB/Webcam (or RPi fallback)
        return f"v4l2src device=/dev/video{source} ! videoscale ! videoconvert ! video/x-raw,format=BGR ! appsink name=sink"
