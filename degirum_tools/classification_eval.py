...
def model_prediction(model, validation_images_dir, label, labelsmap, k=1):
    ...
    val_data,prediction_score, prediction=[],[],[]
    for img in tqdm(all_images, desc="Processing Images"):
        ...
        if os.path.isfile(img):
            with model:
                res = model(img)
                sorted_predictions = sorted(res.results, key=lambda x: x['score'], reverse=True)
                top_predictions = sorted_predictions[:k] ## get the top k predictions
                top_labels = [pred['label'] for pred in top_predictions]
                prediction.append([labelsmap[top_labels[i]] for i in range(len(top_labels))]) 
...
class ImageClassificationModelEvaluator:
    ...
    def evaluate(self,
        ...
        for label in os.listdir(image_folder_path):
            gndtruth = []
            data_per_class = count_files_in_directory(os.path.join(image_folder_path, label))
            ...
            prediction, val_data = model_prediction(self.dg_model, image_folder_path, label, self.classmap, k=self.top_k)

            misclassified_images = identify_misclassified_examples(prediction, gndtruth, val_data)
