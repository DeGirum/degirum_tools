...
def test_bf():
    ...
    res_list = [{"bbox": [10, 40, 20, 50], "score": 0.8, "category_id": "1"},
    ...
    res_list_2 = [{"bbox": [0, 0, 10, 10], "score": 0.8, "category_id": "1"},
    ...
    fusion_result = edge_box_fusion([res_list[0], res_list[4]], 0.8, 0.3, destructive=False)
    assert len(fusion_result) == 2
...
def test_generate_tiles():
    ...
    tile_extractor = TileExtractorPseudoModel(2, 2, 0.1, dummy_model)
    results = list(tile_extractor.predict_batch([np.zeros((1000, 900, 3), dtype=np.uint8)]))[0]
    tiles = [x['bbox'] for x in results.results]
