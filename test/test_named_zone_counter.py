...
def test_entry_delay_frames():
    ...
    assert result1.zone_counts["test_zone"]["total"] == 0, "Frame 1: Should not count yet (1/3)"
    ...
    assert result2.zone_counts["test_zone"]["total"] == 0, "Frame 2: Should not count yet (2/3)"
    ...
    assert result3.zone_counts["test_zone"]["total"] == 1, "Frame 3: Should be counted now"
    ...
    occupied_events = [e for e in result3.zone_events if e['event_type'] == 'zone_occupied']
    ...
    assert exit_events[0]['dwell_time'] is not None, "Frame 6: Exit should have dwell_time"
...
def test_multi_zone_tracking():
    ...
    counter.analyze(result2)
    assert result2.zone_counts["zone_A"]["total"] == 1, "Zone A should still count (grace period, exit_delay=1)"
    assert result2.zone_counts["zone_B"]["total"] == 1, "Zone B should still have 1 object"
    ...
    print("✓ Frame 2: Object 1 in zone_C (entered) + zone_A (grace period), Object 2 in B")
    ...
    assert result3.zone_counts["zone_B"]["total"] == 1, "Zone B should still have 1 object"
...
def test_class_filtering():
    ...
    result = MockResult([
        {"bbox": [120, 120, 170, 170], "track_id": 1, "label": "person"},
        {"bbox": [180, 180, 230, 230], "track_id": 2, "label": "car"},  # Should be ignored
        {"bbox": [240, 240, 290, 290], "track_id": 3, "label": "person"},
    ], frame_number=1)
    counter.analyze(result)
    
    assert result.zone_counts["test_zone"]["total"] == 2, "Should only count persons (2)"
...
def test_re_entry():
    ...
    result1 = MockResult([{"bbox": [150, 150, 200, 200], "track_id": 1, "label": "person"}], 1)
    counter.analyze(result1)
    print("✓ Frame 1: Entry started (1/2)")
    
    result2 = MockResult([{"bbox": [150, 150, 200, 200], "track_id": 1, "label": "person"}], 2)
    counter.analyze(result2)
    ...
    result5 = MockResult([{"bbox": [150, 150, 200, 200], "track_id": 1, "label": "person"}], 5)
    counter.analyze(result5)
    assert result5.zone_counts["test_zone"]["total"] == 0, "Not yet re-established (1/2)"
    print("✓ Frame 5: Re-entry started (1/2)")
    
    result6 = MockResult([{"bbox": [150, 150, 200, 200], "track_id": 1, "label": "person"}], 6)
    counter.analyze(result6)
...
def test_simultaneous_objects():
    ...
    occupied_events = [e for e in result1.zone_events if e['event_type'] == 'zone_occupied']
...
def test_empty_frames():
    ...
    result3 = MockResult([], 3)
    counter.analyze(result3)
    assert result3.zone_counts["test_zone"]["total"] == 1, "Should still count (grace period)"
...
def test_occupancy_transitions():
    ...
    occupied_events = [e for e in result2.zone_events if e['event_type'] == 'zone_occupied']
    ...
    result3 = MockResult([{"bbox": [150, 150, 200, 200], "track_id": 1, "label": "person"}], 3)
    counter.analyze(result3)
    ...
    result4 = MockResult([], 4)
    counter.analyze(result4)
    assert result4.zone_counts["test_zone"]["total"] == 1, "Still counted (exit_delay 2/2)"
    ...
    counter.analyze(result5)
    assert result5.zone_counts["test_zone"]["total"] == 1, "Still counted (exit_delay 1/2)"
    ...
    empty_events = [e for e in result6.zone_events if e['event_type'] == 'zone_empty']
    occupied_events = [e for e in result6.zone_events if e['event_type'] == 'zone_occupied']
    assert len(empty_events) == 1, "Should have zone_empty event"
    assert len(occupied_events) == 0, "No zone_occupied event"
    assert empty_events[0]['dwell_time'] is not None, "Empty event should have dwell_time"
    print(f"✓ Frame 6: Grace period expired - zone_empty event fired (dwell={empty_events[0]['dwell_time']:.3f}s)")
...
def test_occupancy_no_flicker():
    ...
    result2 = MockResult([], 2)
    counter.analyze(result2)
    assert result2.zone_counts["test_zone"]["total"] == 1, "Should still count (grace period)"
    ...
    print("✓ Frame 3: Object reappears - no zone_occupied flicker (grace period worked)")
...
def test_occupancy_multiple_objects():
    ...
    counter.analyze(result2)
    assert result2.zone_counts["test_zone"]["total"] == 2, "Both still counted (obj 1 in grace)"
    empty_events = [e for e in result2.zone_events if e['event_type'] == 'zone_empty']
    assert len(empty_events) == 0, "Zone still occupied (obj 2 present)"
    print("✓ Frame 2: Object 1 missing (grace period), object 2 present - zone stays occupied")
    ...
    print("✓ Frame 3: Object 1 exited, object 2 remains - zone STAYS occupied (no flicker)")
...
def test_bbox_annotation_fields():
    ...
    result = MockResult([{"bbox": [150, 150, 200, 200], "track_id": 1, "label": "person"}], 1)
    counter.analyze(result)
