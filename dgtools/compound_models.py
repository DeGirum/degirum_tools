...
class CompoundModelBase(ABC):
    ...
    def predict_batch(self, data):
        ...
        Returns:
            Generator object which iterates over inference result objects returned by the second model.
...
class CroppingCompoundModel(CompoundModelBase):
    ...
    def find_bbox(self, results, image_shape):
        """
...
class CroppingAndClassifyingCompoundModel(CroppingCompoundModel):
    ...
    def transform_result2(self, result2):
        ...
        idx = self.find_bbox(result2.info.results, result2.image.shape)
        if idx >= 0:
